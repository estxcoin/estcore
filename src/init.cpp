 ...
 */
static const char* BITCOIN_PID_FILENAME = "estxd.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/estxproject/estx>";
    const std::string URL_WEBSITE = "<https://estx.org>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateEstx");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                dev::eth::Ethash::init();
                fs::path estxStateDir = GetDataDir() / "stateEstx";
                bool fStatus = fs::exists(estxStateDir);
                const std::string dirEstx(estxStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsEstxstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<EstxState>(new EstxState(dev::u256(0), EstxState::openDB(dirEstx, hashDB, dev::WithExisting::Trust), dirEstx, existsEstxstate));
                dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::estxMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(estxStateDir.string()));
                if (fReset) {
