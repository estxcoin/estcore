...
inline void initState(){
    ...
    const std::string dirEstx = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<EstxState>(new EstxState(dev::u256(0), EstxState::openDB(dirEstx, hashDB, dev::WithExisting::Trust), dirEstx + "/estxDB", dev::eth::BaseState::Empty));
...
inline dev::Address createEstxAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline EstxTransaction createEstxTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    EstxTransaction txEth;
    if(recipient == dev::Address()){
        txEth = EstxTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = EstxTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<EstxTransaction> txs){
