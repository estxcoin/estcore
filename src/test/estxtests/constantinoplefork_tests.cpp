...
void genesisLoading(){
    ...
    dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::estxMainNetwork, 999)));
...
BOOST_AUTO_TEST_CASE(checking_returndata_opcode_after_fork){
    ...
    txs.push_back(createEstxTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address()));
    txs.push_back(createEstxTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++HASHTX, dev::Address()));
    executeBC(txs);

    // Call upgrade to
    dev::Address proxy = createEstxAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<EstxTransaction> txsCall;
    txsCall.push_back(createEstxTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++HASHTX, proxy));
    executeBC(txsCall);

    // Call mint
    std::vector<EstxTransaction> txsMint;
    txsMint.push_back(createEstxTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++HASHTX, proxy));
    auto result = executeBC(txsMint);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);

    // Call balance of
    std::vector<EstxTransaction> txsbalance;
    txsbalance.push_back(createEstxTransaction(ParseHex("70a082310000000000000000000000000101010101010101010101010101010101010101"), 0, GASLIMIT, dev::u256(1), ++HASHTX, proxy));
...
BOOST_AUTO_TEST_CASE(checking_constantinople_after_fork){
    ...
    // Create contract
    std::vector<EstxTransaction> txs;
    txs.push_back(createEstxTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address()));
    executeBC(txs);

    // Call is it constantinople
    dev::Address proxy = createEstxAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<EstxTransaction> txIsItConstantinople;
    txIsItConstantinople.push_back(createEstxTransaction(CODE[5], 0, GASLIMIT, dev::u256(1), ++HASHTX, proxy));
