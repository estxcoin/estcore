...
void checkResult(bool isCreation, std::vector<EstxTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    EstxTxConverter converter(transaction, NULL);
    ExtractEstxTX estxTx;
    BOOST_CHECK(converter.extractionEstxTransactions(estxTx));
    std::vector<EstxTransaction> result = estxTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(estxtxconverter_tests, TestingSetup)
