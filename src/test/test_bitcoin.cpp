...
BasicTestingSetup::BasicTestingSetup(const std::string& chainName)
    : m_path_root(fs::temp_directory_path() / "test_estx" / strprintf("%lu_%i", (unsigned long)GetTime(), (int)(InsecureRandRange(1 << 30))))
{
    SHA256AutoDetect();
    ECC_Start();
    SetupEnvironment();
...
////////////////////////////////////////////////////////////// estx
        dev::eth::Ethash::init();		
        boost::filesystem::path pathTemp = fs::temp_directory_path() / strprintf("test_estx_%lu_%i", (unsigned long)GetTime(), (int)(GetRand(100000)));
        boost::filesystem::create_directories(pathTemp);
        const dev::h256 hashDB(dev::sha3(dev::rlp("")));
        globalState = std::unique_ptr<EstxState>(new EstxState(dev::u256(0), EstxState::openDB(pathTemp.string(), hashDB, dev::WithExisting::Trust), pathTemp.string(), dev::eth::BaseState::Empty));
        dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::estxTestNetwork)));
