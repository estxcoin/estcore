...
void EstxDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool EstxDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule EstxDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t EstxDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t EstxDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t EstxDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t EstxDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool EstxDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void EstxDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void EstxDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void EstxDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void EstxDGP::createParamsInstance(){
...
dev::Address EstxDGP::getAddressForBlock(unsigned int blockHeight){
...
void EstxDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void EstxDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void EstxDGP::parseStorageOneUint64(uint64_t& value){
...
void EstxDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule EstxDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
