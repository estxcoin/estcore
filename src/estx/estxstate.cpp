...
#include <estx/estxstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

EstxState::EstxState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = EstxState::openDB(_path + "/estxDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

EstxState::EstxState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute EstxState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, EstxTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createEstxAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            estx::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> EstxState::vins() const // temp
{
...
void EstxState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* EstxState::vin(dev::Address const& _a) const
{
    return const_cast<EstxState*>(this)->vin(_a);
}

Vin* EstxState::vin(dev::Address const& _addr)
{
...
// void EstxState::commit(CommitBehaviour _commitBehaviour)
// {
...
void EstxState::kill(dev::Address _addr)
{
...
void EstxState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void EstxState::deleteAccounts(std::set<dev::Address>& addrs){
...
void EstxState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void EstxState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void EstxState::validateTransfersWithChangeLog(){
