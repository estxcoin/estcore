...
const char * const BITCOIN_CONF_FILENAME = "estx.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Estx
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Estx
    // Mac: ~/Library/Application Support/Estx
    // Unix: ~/.estx
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Estx";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Estx";
#else
    // Unix
    return pathRet / ".estx";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Estx Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Estx Core developers";
    }
