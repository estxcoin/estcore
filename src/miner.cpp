...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setEstxSchedule(estxDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = estxDGP.getBlockSize(nHeight);
    minGasPrice = estxDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = estxDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    EstxTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractEstxTX resultConverter;
    if(!convert.extractionEstxTransactions(resultConverter)){
    ...
    for(EstxTransaction estxTransaction : estxTransactions){
        txGas += estxTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + estxTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(estxTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, estxTransactions, hardBlockGasLimit, chainActive.Tip());
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "estxstake";
...
void StakeEstxs(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
