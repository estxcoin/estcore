...
#if defined(NDEBUG)
# error "Estx cannot be compiled without assertions."
#endif
...
std::unique_ptr<EstxState> globalState;
...
const std::string strMessageMagic = "Estx Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool rawTx) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            EstxDGP estxDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = estxDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = estxDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            ...
            EstxTxConverter converter(tx, NULL, NULL, contractflags);
            ExtractEstxTX resultConverter;
            if(!converter.extractionEstxTransactions(resultConverter)){
            ...
            std::vector<EstxTransaction> estxTransactions = resultConverter.first;
            std::vector<EthTransactionParams> estxETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(EstxTransaction estxTransaction : estxTransactions){
                sumGas += estxTransaction.gas() * estxTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, estxTransaction.gasPrice());
                } else {
                    txMinGasPrice = estxTransaction.gasPrice();
                }
                VersionVM v = estxTransaction.getVersion();
                if(v.format!=0)
                ...
                if(estxTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(estxTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(estxTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)estxTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(estxETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > estxTransactions.size())
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s ESTX additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // estx
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // estx
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    EstxTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<EstxTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path estxDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(estxDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(estxDir.string(), std::ios::in | std::ios::out);
...
bool EstxTxConverter::extractionEstxTransactions(ExtractEstxTX& estxtx){
    ...
    std::vector<EstxTransaction> resultTX;
    ...
    estxtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool EstxTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool EstxTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
EstxTransaction EstxTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    EstxTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = EstxTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = EstxTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t EstxTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // estx
    EstxDGP estxDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setEstxSchedule(estxDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = estxDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = estxDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = estxDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // estx
...
///////////////////////////////////////////////////////////////////////////////////////// estx
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            EstxTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractEstxTX resultConvertEstxTX;
            if(!convert.extractionEstxTransactions(resultConvertEstxTX)){
            ...
            if(!CheckMinGasPrice(resultConvertEstxTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertEstxTX.first, blockGasLimit, pindex->pprev);
            ...
            for(EstxTransaction& qtx : resultConvertEstxTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertEstxTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertEstxTX.first[k].from(), resultConvertEstxTX.first[k].to(),
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // estx
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // estx
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // estx
            globalState->setRootUTXO(oldHashUTXORoot); // estx
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // estx
        uint32_t sizeBlockDGP = estxDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Estx PoS we need to sync this after every block to ensure txdb is populated for
