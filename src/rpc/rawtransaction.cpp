...
{
    // Call into TxToUniv() in estx-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in estx-common, so we query them here and push the
                            ...
                            {"contract", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                                    ...
                                    {"amount", RPCArg::Type::AMOUNT,  /* default */ "0", "Value in ESTX to send with the call, should be a valid amount, default 0"},
...
static UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    const RPCHelpMan help{"decoderawtransaction",
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"Q2tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) estx address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"decodepsbt",
                "\nReturn a JSON object representing the serialized, base64-encoded partially signed Estx transaction.\n",
                {
            ...
            "          \"hex\" : \"hex\",            (string) The hex\n"
            "          \"type\" : \"pubkeyhash\",    (string) The type, eg 'pubkeyhash'\n"
            "          \"address\" : \"address\"     (string) Estx address if there is one\n"
            "        }\n"
            "      },\n"
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"combinepsbt",
                "\nCombine multiple partially signed Estx transactions into one transaction.\n"
