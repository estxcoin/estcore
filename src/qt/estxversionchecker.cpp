...
#define paternVersion "estx-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

EstxVersionChecker::EstxVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

EstxVersionChecker::~EstxVersionChecker()
{

}

bool EstxVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> EstxVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(ESTX_RELEASES)));
    QEventLoop event;
...
Version EstxVersionChecker::getMaxReleaseVersion()
{
