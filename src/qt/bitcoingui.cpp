...
#include <qt/estxversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    estxVersionChecker = new EstxVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Estx address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and estx: URIs)"));
    ...
    historyAction = new QAction(platformStyle->MultiStatesIcon(":/icons/history"), tr("&Transactions"), this);//ESTX_LINE
    ...
    SRCTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Estx addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Estx addresses"));
    ...
    openAction->setStatusTip(tr("Open a estx: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Estx command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->mapGroup(SRCTokenAction, tokenActions);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && estxVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(ESTX_RELEASES, ESTX_RELEASES);
            QString message(tr("New version of Estx wallet is available on the Estx source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Estx network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("Estx"); // default title
