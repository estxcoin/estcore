    ...
    model(nullptr)
{
    ...
    connect(ui->btnSaveAs, &QPushButton::clicked, ui->lblSRCode, &QRImageWidget::saveImage);
}
...
bool ReceiveRequestDialog::createSRCode(QLabel *label, SendCoinsRecipient _info, bool showAddress)
{
#ifdef USE_SRCODE
...
#ifdef USE_SRCODE
    if(createSRCode(ui->lblSRCode, info))
    {
