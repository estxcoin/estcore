...
bool ToHash160(const std::string& strEstxAddress, std::string& strHash160)
{
    CTxDestination estxAddress = DecodeDestination(strEstxAddress);
    if(!IsValidDestination(estxAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&estxAddress);
    if(keyid){
...
bool ToEstxAddress(const std::string& strHash160, std::string& strEstxAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination estxAddress = keyid;
    if(IsValidDestination(estxAddress)){
        strEstxAddress = EncodeDestination(estxAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToEstxAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToEstxAddress(tokenEvent.receiver, tokenEvent.receiver);
