...
void CreateContract::setLinkLabels()
{
    ...
    ui->labelSolidity->setText("<a href=\"https://qmix.estx.org/\">Solidity compiler</a>");
}
...
void CreateContract::setModel(WalletModel *_model)
{
    ...
    // update the display unit, to not use the default ("ESTX")
    updateDisplayUnit();
}
...
void CreateContract::on_gasInfoChanged(quint64 blockGasLimit, quint64 minGasPrice, quint64 nGasPrice)
{
    ...
    ui->labelGasPrice->setToolTip(tr("Gas price: ESTX price per gas unit. Default = %1, Min = %2").arg(QString::fromStdString(FormatMoney(DEFAULT_GAS_PRICE))).arg(QString::fromStdString(FormatMoney(minGasPrice))));
