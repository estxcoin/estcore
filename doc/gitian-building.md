...
*Setup instructions for a Gitian build of Estx Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Estx
...
- [Building Estx Core](#building-estx-core)
...
Clone the git repositories for estx and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/estxproject/estx --recursive
git clone https://github.com/estxproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/estx/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the estx repository under 'Fetch and create inputs' to install sources which require
...
To build Estx Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the estx repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/estx/.git/
...
For example:
```bash
URL=https://github.com/laanwj/estx.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit estx=${COMMIT} --url estx=${URL} ../estx/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit estx=${COMMIT} --url estx=${URL} ../estx/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit estx=${COMMIT} --url estx=${URL} ../estx/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-estx:
./bin/gbuild --commit estx=${COMMIT},cpp-eth-estx=${ETHCOMMIT} --url estx=${URL},cpp-eth-estx=${ETHURL} ../estx/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the estx git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../estx/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/estxproject/estx-detached-sigs.git

BTCPATH=/some/root/path/estx
SIGPATH=/some/root/path/estx-detached-sigs

./bin/gbuild --url estx=${BTCPATH},signature=${SIGPATH} ../estx/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/estx-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/estx-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/estx-osx-build.assert
```
...
[estxproject/gitian.sigs](https://github.com/estxproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@estx.org) and he will commit them.
